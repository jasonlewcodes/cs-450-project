...
class Visualization1 extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.data !== this.props.data || prevState.depressionFilter !== this.state.depressionFilter || prevState.genderFilter !== this.state.genderFilter) {
      this.renderChart();
    }
  }
  ...
  getFilteredData =()=> {
    ...
    return data.filter((d) => {
      const depressionMatch = depressionFilter === "All" ||
       (depressionFilter === "depression" && +d.depression > 0) ||
    (depressionFilter === "no_depression" && +d.depression === 0);

      const genderMatch =genderFilter === "All" ||
        (genderFilter === "male" && d.gender.toLowerCase() === "male") ||
      ...
      <div style={{ display: "flex", alignItems: "flex-start" }}>
        ...
        <div style={{ marginLeft: "10px", marginTop: "80px" }}>
          ...
          <div style={{ marginBottom: "20px" }}>
            ...
            <label>
              <input type="radio" name="depressionFilter" value="depression"
                checked={this.state.depressionFilter === "depression"}
                onChange={this.UpdateDepressionFilter}/> Depression
            </label><br />
            <label>
              <input type="radio" name="depressionFilter" value="no_depression"
                checked={this.state.depressionFilter === "no_depression"}
                onChange={this.UpdateDepressionFilter}/> No Depression
            </label>
          </div>
          ...
          <div>
            ...
            <label>
              <input type="radio" name="genderFilter" value="female"
                checked={this.state.genderFilter === "female"}
                onChange={this.UpdateGenderFilter}/> Female
            </label>
          </div>
        </div>
      </div>
